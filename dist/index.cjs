...
const debug = {
  hmr: Debug__default("vite-plugin-react-easy-model:hmr"),
  search: Debug__default("vite-plugin-react-easy-model:search")
};
...
async function checkFileAlive(filePath) {
  try {
    await fs__default.stat(filePath);
    return true;
  } catch (error) {
    return false;
  }
...
async function createDeclarations(ctx) {
  ...
  const buildFile = async (filePath) => {
    ...
    return new Promise((resolve, reject) => {
      const child = child_process.exec(
        `tsc ${filePath} --outDir ${path__default.resolve(
          __dirname$1,
          "./cache"
  ...
  const fileUrl = path__default.resolve(
    __dirname$1,
    "./.cache.tsx"
  );
  await fs__default.writeFile(fileUrl, code, {
    // 不存在则创建
    flag: "w+",
    encoding: "utf-8"
  });
  ...
  let content = await fs__default.readFile(cacheUrl, {
    encoding: "utf-8"
  });
...
`;
  ...
  await fs__default.writeFile(
    path__default.resolve(__dirname$1, "../client.d.ts"),
    `
...
class ModelContext {
  ...
  setupWater(watcher) {
    const searchNew = async (changePath) => {
      const actualPath = vite.normalizePath(changePath);
      const rootDirs = this.options.dirs.map(
        (dir) => vite.normalizePath(path__default$1.resolve(this.root, dir))
      );
      ...
      const models = await __privateMethod(this, _searchGlob, searchGlob_fn).call(this, [path__default$1.relative(this.root, actualPath)], father);
      if (models.length) {
...
writeCode_fn = async function(needUpdate = false) {
  ...
  const template = await fs__default.readFile(
    new URL("./templates/index.tsx", (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))),
    {
      encoding: "utf-8"
    }
  );
  const s = new MagicString__default(template);
  let importBanner = "";
  let modelsBody = "const models = {";
  __privateGet(this, _models).forEach((model, index) => {
    const transformPath = removeExt(model.path).split(path__default$1.sep).join("/");
...
searchGlob_fn = async function(dirs, rootDir) {
  ...
  await Promise.all(
    actualDirs.map(async (dir) => {
      ...
      const currentPath = vite.normalizePath(
        path__default$1.join(this.root, dir)
      );
      try {
        await fs__default.access(currentPath);
      } catch (error) {
        debug.search(`${currentPath} not found`);
        return;
      }
      const ignorePath = vite.normalizePath(
        rootDir ?? path__default$1.join(this.root, dir)
      );
      const basename = path__default$1.basename(currentPath);
      const stat = await fs__default.stat(currentPath);
      if (stat.isDirectory()) {
        const children = await fs__default.readdir(currentPath);
        const childSearch = await __privateMethod(this, _searchGlob, searchGlob_fn).call(this, children.map((v) => path__default$1.join(dir, v)), ignorePath);
        r.push(...childSearch);
      } else {
        if (
          // 如果当前文件名为model，则直接返回
          ...
          const code = await fs__default.readFile(currentPath, {
            encoding: "utf-8"
          });
          try {
          ...
          const relativePath = path__default$1.relative(
            ignorePath,
            currentPath
          );
          r.push({
...
containerModelsDir_fn = function(inputPath) {
  const pathSegments = inputPath.split(path__default$1.sep);
...
transformPathToNamespace_fn = function(relativePath) {
  ...
  return removeSuffixPath.split(path__default$1.sep).filter((path2) => path2 !== "models").join(".");
};
  ...
  return {
    ...
    async load(id, options2) {
      if (id === virtualModuleId) {
        ...
        const { code, map } = await vite.transformWithEsbuild(
          ctx.code,
          "",
          {
            loader: "tsx"
